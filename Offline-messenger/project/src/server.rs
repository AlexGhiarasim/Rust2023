...
impl Commands {
    ...
    fn reply(&mut self, stream: &mut TcpStream) {
        ...
        if let Ok(file) = File::open(&path) {
            ...
            // Parcurge liniile în ordine inversă de la coadă
            for line in messages.iter().rev() {
                if line.contains(user.unwrap_or("")) {
                    // Verifică dacă mesajul conține utilizatorul căutat
                    if index_msg == 1 {
                        // Dacă x == 1, atunci am găsit mesajul căutat
                        ...
                        return; // Ieșim din funcție după găsirea mesajului căutat
                    } else {
                        // Scade x cu 1 pentru a urmări numărul de apariții căutate
                        index_msg -= 1;
                    }
                }
            }
    
            // Dacă am ajuns aici, înseamnă că nu s-a găsit mesajul căutat
            if let Err(err) = stream.write_all(b"\nMessage not found!\n") {
                eprintln!("Error writing to client: {}", err);
            }
        } else {
...
fn delete_client(socket_addr: std::net::SocketAddr) -> std::io::Result<()> {
    // Citeste continutul fisierului intr-un vector de linii
    let filename = "clienti_conectati.txt";
    let file_content = fs::read_to_string(filename)?;

    // Creeaza un vector cu linii separate
    let lines: Vec<&str> = file_content.lines().collect();
    ...
    for line in lines {
        // Parseaza adresa IP si portul din linie
        if let Ok(parsed_addr) = line.parse::<std::net::SocketAddr>() {
            // Verifica daca adresa din linie este diferita de cea primita ca parametru
            if parsed_addr != socket_addr {
                // Scrie linia inapoi in fisier
                writeln!(file, "{}", line)?;
            }
        } else {
            // Daca nu se poate parsa linia, scrie-o inapoi in fisier
            writeln!(file, "{}", line)?;
        }
    }
    Ok(())
...
fn remove_login(socket_addr: SocketAddr, user: String) -> std::io::Result<()> {
    ...
    // Creează un vector pentru a stoca liniile actualizate
    let mut linii_actualizate = Vec::new();
    ...
    for line in reader.lines() {
        ...
        // Verifică dacă linia curentă conține socket_addr și user
        if trimmed_line.starts_with(&format!("{} - {}", socket_addr, user)) {
            // Păstrează doar socket_addr în loc de a continua
            linii_actualizate.push(format!("{}", socket_addr));
        } else {
            linii_actualizate.push(line);
        }
    }
    // Deschide fișierul în modul de scriere pentru a rescrie toate liniile
    let mut file = File::create("clienti_conectati.txt")?;
    // Scrie toate liniile actualizate în fișier
    for line in linii_actualizate {
        writeln!(file, "{}", line)?;
    }
...
fn find_user_in_users(user: String) -> bool {
    ...
    // Create a BufReader<File> instead of BufReader<Result<File, std::io::Error>>.
    let reader = BufReader::new(file);
    ...
    for line in reader.lines() {
        ...
        if let Ok(line) = line {
            ...
            // Print an error message if there's an issue reading a line.
            eprintln!("Error reading line from file");
            return false;
        }
    }
