...
fn set_pixels(canvas: &mut [[char; 50]; 10], coordinates: &[(usize, usize, u8)]) {
    for &(x, y, ch_byte) in coordinates {
        let ch = ch_byte as char;

        if let Some(line) = canvas.get_mut(y) {
            if let Some(pixel) = line.get_mut(x) {
                *pixel = ch;
            }
        }
    }
